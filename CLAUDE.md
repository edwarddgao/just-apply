...
- `scrape_companies.py` — scrapes company ratings/funding from Simplify.js company pages → `companies` table
...
- `agent_prompt.txt` — system prompt for pipeline agents (resume, workflow, known gaps). Generated by `/setup`.
- `agent_prompt.example.txt` — template for `agent_prompt.txt` with placeholder values
